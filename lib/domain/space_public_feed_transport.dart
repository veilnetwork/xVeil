...
const Duration kSpacePublicMediaGrantRequestWindow =
    kSpacePublicFeedRequestWindow;
...
/// A public reader's short-lived request to open the existing content stream
/// for one CID already committed by an exact verified public-feed revision.
///
/// This is deliberately separate from `GroupContentRequest`: public readers
/// are not members and must not acquire membership/ACL authority. A holder
/// grants only the exact `(authenticated requester, contentId)` pair after
/// independently matching [descriptorHash], [manifestHash] and [contentId]
/// against its verified public package.
class SpacePublicMediaGrantRequest {
  ...
  bool isStructurallyValidAt(int nowMs) =>
      _hashPattern.hasMatch(descriptorHash) &&
      _hashPattern.hasMatch(manifestHash) &&
      _hashPattern.hasMatch(contentId) &&
      _hashPattern.hasMatch(nonce) &&
      requesterPublicKey.length == 32 &&
      signature.length == 64 &&
      createdAtMs >= 0 &&
      createdAtMs <= nowMs + kSpacePublicClockSkew.inMilliseconds &&
      nowMs - createdAtMs <= kSpacePublicMediaGrantRequestWindow.inMilliseconds;
  ...
  Uint8List canonicalBytes() => Uint8List.fromList(
    utf8.encode(
      jsonEncode({
        'v': 1,
        'kind': 'xveil.space.public-media-grant-request',
        'space': spaceId.hex,
        'descriptorHash': descriptorHash,
        'manifestHash': manifestHash,
        'contentId': contentId,
  ...
  SpacePublicMediaGrantRequest withSignature(Uint8List value) =>
      SpacePublicMediaGrantRequest(
        spaceId: spaceId,
        descriptorHash: descriptorHash,
        manifestHash: manifestHash,
  ...
  static SpacePublicMediaGrantRequest? fromJson(Object? value) {
    if (value is! Map ||
        !_hasOnlyKeys(value, const {
          'v',
          'kind',
        ...
        value['v'] != 1 ||
        value['kind'] != 'xveil.space.public-media-grant-request' ||
        value['space'] is! String ||
        value['descriptorHash'] is! String ||
        value['manifestHash'] is! String ||
        value['contentId'] is! String ||
        value['requester'] is! String ||
        value['requesterKey'] is! String ||
        value['nonce'] is! String ||
        value['createdAt'] is! int ||
    ...
    try {
      return SpacePublicMediaGrantRequest(
        spaceId: NodeId.fromHex(value['space'] as String),
        descriptorHash: value['descriptorHash'] as String,
        manifestHash: value['manifestHash'] as String,
        contentId: value['contentId'] as String,
        requester: NodeId.fromHex(value['requester'] as String),
        requesterPublicKey: Uint8List.fromList(
          base64Decode(value['requesterKey'] as String),
        ),
