...
void main() {
  test(
    'a fileNack only re-sends a blob within the REQUESTING peer\'s '
    ...
    () async {
      ...
      final secret = Uint8List.fromList(
        List.generate(7000, (i) => (i * 11) % 256),
      );
      await mA.sendFile(c, secret, 'for-C-only.bin');
      await _settle();
      final tid = (await sA.loadMessages(c.hex)).firstWhere((m) => m.isFile).id;
      ...
      expect(
        tA.sends.where((s) => s.env.kind == WireKind.fileChunk),
        isEmpty,
  ...
  test('a fresh-tid fileNack flood neither re-sends a blob nor grows the '
    ...
    for (var i = 0; i < 50; i++) {
      tA.inject(
        b,
        fileNackEnvelope(transferId: 'ghost-$i', missing: null).encode(),
      );
    }
    await _settle();
    expect(
