...
void main() {
  ...
  test(
    'F resolves R\'s relay X25519 cross-node on the local mesh',
    ...
    () async {
      ...
      try {
        ...
        while (resolved == null && attempts < 25) {
          attempts++;
          resolved = (await clientF.lookupRelayX25519(rId))?.key;
          stderr.writeln(
            '[xnode] attempt $attempts: '
        ...
        stderr.writeln(
          '[xnode] ✓ cross-node relay X25519 resolved on the local mesh',
        );
      } finally {
        await clientF.close();
        await clientR.close();
