...
void main() {
  ...
  test(
    ...
    () async {
      ...
      final runtimeDir = Directory.systemTemp
          .createTempSync('xveil-relaykey-testnet-')
          .path;
      ...
      try {
        ...
        while (resolved == null && attempts < 18) {
          attempts++;
          try {
            final r = (await client.lookupRelayX25519(relayId))?.key;
            stderr.writeln(
              '[relaykey-testnet] attempt $attempts: '
        ...
        stderr.writeln(
          '[relaykey-testnet] resolved after $attempts attempt(s): '
        ...
        expect(
          ...
          isNotNull,
          reason:
              'could not resolve the peer relay X25519 cross-node over the '
              'testnet DHT (RelayKeyRecord may not be replicated yet)',
        );
        expect(resolved!.length, 32);
        stderr.writeln(
          '[relaykey-testnet] ✓ cross-node relay X25519 resolved over the testnet',
        );
      } finally {
        await client?.close();
