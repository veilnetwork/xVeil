...
void main() {
  ...
  test(
    'persists identity + messages to a real container, survives reopen',
    () async {
      ...
      try {
        ...
        expect(
          await storage.open(password: 'sw0rdfish', createIfMissing: true),
          isTrue,
        );
  ...
  test(
    'delete leaves a reclaimable orphan; scrub reclaims it (real container)',
    () async {
      ...
      try {
        ...
        final storage = HiddenVolumeStorage(
          ({required password, required create}) => store,
        );
        ...
        expect(
          await storage.loadMessages(conv),
          isEmpty,
  ...
  test(
    'deleting a file message purges + reclaims the blob (real container)',
    () async {
      ...
      try {
        ...
        await storage.storeFile(
          'blob1',
          Uint8List.fromList(List.filled(4000, 42)),
          name: 'secret.bin',
        );
        await storage.appendMessage(
          Message(
        ...
        await storage.deleteMessage(
          conv,
          'fmsg',
        ); // folds the blob purge in one commit
        expect(
          await storage.loadFile('blob1'),
          isNull,
          reason: 'blob row gone',
        );

        expect(
          space.vacuumDataBatches(),
          greaterThan(0),
  ...
  // test passed falsely).
  test(
    'stores + reloads a multi-MiB INCOMPRESSIBLE file on a real container',
    () async {
      ...
      try {
        ...
        final data = Uint8List.fromList(
          List.generate(3000000, (_) => rnd.nextInt(256)),
        );
  ...
  test(
    'lean padding sharply reduces future small-write file growth',
    () async {
      final dir = Directory.systemTemp.createTempSync('xveil_hv_padding_');
      try {
        Future<int> growthFor(hv.PaddingPreset preset) async {
          final path =
              '${dir.path}/${preset == hv.PaddingPreset.none ? 'lean' : 'bucket'}.store';
          final storage = HiddenVolumeStorage(
            hvSpaceOpener(
              path,
              argon: hv.ArgonPreset.min,
          ...
          await storage.close();
          return File(path).lengthSync() - before;
        }
  ...
  test('multi-space lean padding reduces shared-container growth', () async {
    final dir = Directory.systemTemp.createTempSync('xveil_hv_mspadding_');
    try {
      Future<int> growthFor(hv.PaddingPreset preset) async {
        final path =
            '${dir.path}/${preset == hv.PaddingPreset.none ? 'lean' : 'bucket'}.store';
        ...
        for (var i = 0; i < 12; i++) {
          backing.commit(id, [
            PutOp(
              Ns.settings,
              Uint8List.fromList('hot-counter'.codeUnits),
              Uint8List.fromList('$i'.codeUnits),
            ),
          ]);
        }
        backing.close();
  ...
  test(
    'master roster + openWithKeys, one space open at a time (real container)',
    () async {
      ...
      try {
        ...
        final child = make();
        expect(
          await child.open(password: 'pw-alice', createIfMissing: true),
          isTrue,
        );
        ...
        final master = make();
        expect(
          await master.open(password: 'pw-master', createIfMissing: true),
          isTrue,
        );
        await master.saveRoster([
          RosterEntry(label: 'alice', spaceKeys: childKeys),
        ]);
        await master.close();
