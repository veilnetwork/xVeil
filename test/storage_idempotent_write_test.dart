...
void main() {
  ...
  group('FileStore.storeFile', () {
    ...
    test('replacing with different bytes purges the old chunks first (sync)',
      ...
      final metaRaw = s.get(Ns.settings, Uint8List.fromList('file:id'.codeUnits));
      ...
      expect(firstOldChunk, isEmpty,
          reason: 'replaced blob chunks must be zeroed, not leaked');
    });

    test('re-storing identical bytes is a no-op (storage async path)',
        () async {
    ...
    test('replacing with different bytes still works (storage async path)',
        () async {
  ...
  group('markMessageStatus', () {
    test('re-marking the stored status does not commit (per-launch re-acks)',
        () async {
  ...
  group('StorageRelayKeyCache across launches', () {
    test('a fresh cache instance does not re-commit a stored fresh key',
        () async {
      ...
      expect(counting.commits, after,
          reason: 're-put of a stored fresh key on a new launch must not '
    ...
    test('a fresh cache instance does not re-commit the same preferred relay',
        () async {
