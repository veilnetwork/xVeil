...
void main() {
  ...
  test(
    'keepAllOnline off → master unlock uses the picker (one-active)',
    () async {
      SharedPreferences.setMockInitialValues({
        'onboarded': true,
        'keep_all_online': false,
      });
      final masterContainer = FakeHvContainer();
  ...
  test(
    'all-online: setIdentityAnonymous toggles a background identity, '
    ...
    () async {
      ...
      MultiIdentitySession build() => MultiIdentitySession(
        SyncWrappedAsyncMultiSpaceBacking(backing),
        runtimeDirBase: '/run',
        listenPortBase: 9000,
        boot: (spec, storage) async =>
      ...
      final bob = (await check.loadRoster())!.firstWhere(
        (e) => e.label == 'bob',
      );
      await check.close();
