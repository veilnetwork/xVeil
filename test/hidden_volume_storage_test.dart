...
void main() {
  ...
  test('messages append to the log and read back in time order', () async {
    ...
    // New profiles never consume the bounded legacy index: two messages use
    // distinct global ids inside the first deterministic shard.
  ...
  test(
    ...
    () async {
      expect(
        Ns.messageLogNamespaceFor(Ns.messageLogShardCapacity),
        Ns.messageLogShardLast,
      );
      expect(
        () => Ns.messageLogNamespaceFor(Ns.messageLogShardCapacity + 1),
        throwsRangeError,
      );
      final boundaryStore = FakeKvLogStore();
      boundaryStore.commit([
        PutOp(
          Ns.settings,
          Uint8List.fromList(utf8.encode('msg_next_id')),
          Uint8List.fromList(utf8.encode('${Ns.messageLogShardSize}')),
        ),
      ]);
      final boundaryStorage = HiddenVolumeStorage(
        ({required Uint8List password, required bool create}) => boundaryStore,
      );
      await boundaryStorage.open(password: 'pw');
      final conv = _id(42).hex;
      await Future.wait([
        for (var i = 0; i < 4; i++)
          boundaryStorage.appendMessage(
            Message(
              id: 'boundary-$i',
              conversationId: conv,
              direction: MessageDirection.outgoing,
              body: 'boundary-$i',
              timestamp: DateTime(2026, 2, 1, 0, i),
              author: conv,
              seq: i + 1,
            ),
          ),
      ...
      expect(boundaryStore.count(Ns.messageLogShardFirst), 1);
      expect(boundaryStore.count(Ns.messageLogShardFirst + 1), 3);
      expect(
        (await boundaryStorage.loadMessages(conv)).map((m) => m.id).toSet(),
        {for (var i = 0; i < 4; i++) 'boundary-$i'},
      );

      // A fresh handle discovers both shards solely from durable markers.
      final fresh = HiddenVolumeStorage(
        ({required Uint8List password, required bool create}) => boundaryStore,
      );
      await fresh.open(password: 'pw');
      expect((await fresh.loadMessages(conv)).length, 4);
    },
  );
