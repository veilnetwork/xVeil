...
void main() {
  ...
  test(
    'a large file routes to the ENCRYPTED on-disk tier (out-of-order pieces; '
    'ranged + whole reads); a small file stays in the volume',
    () async {
      ...
      expect(
        await s.readFileRange('big', 800, 400),
        Uint8List.sublistView(whole, 800, 1200),
        reason: 'spans piece 0|1',
      );
      ...
      expect(
        files,
        isNotEmpty,
        reason: 'encrypted pieces written to the FS tier',
      );
      expect(
        await files.first.readAsBytes(),
        isNot(equals(piece(0))),
      ...
      await s.storeFilePiece(
        ...
        50,
        50,
        Uint8List.sublistView(whole, 0, 50),
        name: 's.bin',
      );
      expect(await s.hasFile('small'), isTrue);
      expect(await s.loadFile('small'), Uint8List.sublistView(whole, 0, 50));
      expect(
        blobDir.listSync(recursive: true).whereType<File>().length,
        onDiskCount,
        reason: 'small file did not write to the on-disk tier',
      );
    },
  );
  ...
  Future<String> storeOnDiskFileMessage(
    ...
    await s.storeFilePiece(
      ...
      pieceSize,
      total,
      Uint8List.sublistView(bytes, 0, 1000),
      name: 'big.bin',
    );
    await s.storeFilePiece(
      cid,
      ...
      pieceSize,
      total,
      Uint8List.sublistView(bytes, 1000),
    );
    expect(await s.hasFile(cid), isTrue);
    final m = await s.appendMessage(
      Message(
        id: 'msg-$cid',
  ...
  test('deleting a large-file message scrubs the on-disk tier: key row gone '
  ...
  test('settings GC removes orphan on-disk blobs and metadata', () async {
    final s = HiddenVolumeStorage(_mem());
    await s.open(password: 'p', createIfMissing: true);
    s.useOnDiskTier(blobDir, minBytes: 1000);
    final peer = id(0xBC);
    await storeOnDiskFileMessage(s, peer, 'cid-orphan');
    expect(await s.hasFile('cid-orphan'), isTrue);
    expect(blobDir.listSync(recursive: true).whereType<File>(), isNotEmpty);

    await s.purgeMessageLog(); // leaves ondisk:cid-orphan until GC.
    final swept = await s.sweepSettingsGarbage();

    expect(swept, greaterThan(0));
    expect(await s.hasFile('cid-orphan'), isFalse);
    expect(blobDir.listSync(recursive: true).whereType<File>(), isEmpty);
  });
  ...
  test(
    ...
    () async {
      ...
      expect(
        ...
        before + 1,
        reason:
            'one constant-size meta row per file — NOT one padded commit '
            'per received piece',
      );
      expect(
        await s.hasFile('many'),
        isTrue,
  ...
  test('adaptive piece size scales but is CAPPED — a TB-scale file grows the '
    ...
    final pieces =
        (1024 * gib) ~/ MessagingService.adaptivePieceSize(1024 * gib);
    expect(
      pieces,
      32768,
      reason:
          '1 TB ≈ 32 K piece hashes ≈ a ~3 MB manifest — still under '
