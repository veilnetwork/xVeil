...
#   XVEIL_SWARM_FINAL_PULLER=A|C         swarm final puller after B restart.
#                                      embedded-endpoints defaults to A (two
#                                      embedded runtimes); external/default
#                                      keeps C for the three-peer swarm shape.
...
cleaned_up=0
lock_acquired=0
SWARM_FINAL_PULLER="$(printf '%s' "${XVEIL_SWARM_FINAL_PULLER:-C}" | tr '[:lower:]' '[:upper:]')"
...
if [[ "$LIVE_TEST" == "swarm" ]]; then
  if [[ "$NODE_MODE" == "embedded-endpoints" && -z "${XVEIL_SWARM_FINAL_PULLER:-}" ]]; then
    SWARM_FINAL_PULLER="A"
  fi
  case "$SWARM_FINAL_PULLER" in
    A|C) ;;
    *)
      echo "XVEIL_SWARM_FINAL_PULLER must be A or C." >&2
      exit 2
      ;;
  esac
  if [[ "$SWARM_FINAL_PULLER" == "C" ]]; then
    peers=(a b c m1 m2)
    app_peers=(a b c)
  fi
  ...
  # For embedded-endpoints, the default final puller is A: A sends to B, B
  # restarts, then A pulls the same content back from B. That exercises durable
  # seeding with only the phone/desktop-shaped two embedded runtimes.
  receiver_peers=(a b)
fi
case "$NODE_MODE" in
  external) ;;
...
cleanup() {
  ...
  local log_peers
  log_peers="$(IFS=,; printf '%s' "${peers[*]}")"
  echo "logs: $NODES/{$log_peers}/node.log"
  exit "$code"
}
...
if [[ "$LIVE_TEST" == "swarm" && "$SWARM_FINAL_PULLER" == "C" ]]; then
...
if [[ "$RUN_TEST" == "1" ]]; then
  ...
  (
    ...
    RUST_LOG="$TEST_RUST_LOG" \
      ...
      XVEIL_SWARM_FINAL_PULLER="$SWARM_FINAL_PULLER" \
